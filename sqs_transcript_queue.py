...
Two queues (create both in AWS first, e.g. via the console or terraform):
    ...
    youtube-rag-transcript-results    — local watcher → bot (optional;
                                          ...
                                          is what bot.py does by default, but
                                          the result queue lets the bot avoid
                                          tight polling loops if you prefer)
"""
...
def enqueue_transcript_request(video_id: str, lang: str) -> None:
    if not REQUEST_QUEUE_URL:
        raise RuntimeError("SQS_REQUEST_QUEUE_URL not set in environment")
    _sqs.send_message(
        QueueUrl=REQUEST_QUEUE_URL,
        MessageBody=json.dumps({"video_id": video_id, "lang": lang}),
    )
    logger.info(f"[sqs] Enqueued transcript request: {video_id}/{lang}")


def receive_transcript_requests(max_messages: int = 1, wait_seconds: int = 20) -> list[dict]:
    """Long-polls the request queue. Returns list of {video_id, lang, _receipt_handle}."""
    if not REQUEST_QUEUE_URL:
        raise RuntimeError("SQS_REQUEST_QUEUE_URL not set in environment")
...
def notify_result(video_id: str, lang: str, status: str, error: str = "") -> None:
